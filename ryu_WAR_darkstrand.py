...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    events = [event.EventSwitchEnter, event.EventPortAdd,event.EventLinkAdd]
    @set_ev_cls(events)
    ...
    def link_fail_handler(self,ev):
        ...
        h1=link.src.dpid
        h2=link.dst.dpid
        if h1 in self.monitoring_list:
            self.monitoring_list.remove(h1)
        if h2 in self.monitoring_list:
            self.monitoring_list.remove(h2)
        ...
        net.add_weighted_edges_from(edges_list)
        nodes = self.net.nodes
        nodes = set(nodes)
        nodes = list(nodes)
        for sw in nodes:
            self.weight[sw] = len(list(set(nx.all_neighbors(self.net, sw))))
        ...
        for key in self.all_pair_shortest_path.keys():
            ...
            if h1 in path and h2 in path:
                ...
                new_path, max_position = self.WAR(index,path,net)
                if new_path == None:
                    print "no alternative path found"
                    continue
                old_path = path[:max_position]
                ...
                if path == back_path:
                    ...
                    match = ofp_parser.OFPMatch(eth_type=self.eth_type, ipv4_src=src, ipv4_dst=dst)
    ...
    def node_failure(self, ev):
        start_time = time.time()
        dpid = ev.switch.dp.id
        print "Node Failure at:", dpid
        ...
        for key in self.all_pair_shortest_path.keys():
            src, dst = key
            if src in [9, 10, 11, 18, 17, 19, 21, 22, 23, 24] and dst in [9, 10, 11, 18, 17, 19, 21, 22, 23, 24]:
                pass
            else:
                continue
            path = self.all_pair_shortest_path[key][0]
            checking = []
            if dpid in path:
                if dpid != path[0] and dpid != path[-1]:
                    prev_sw = path[path.index(dpid)]
                    new_path, max_position = self.WAR(path.index(dpid), path, net)
                    print "Original path:", path
                    if new_path == None:
                        print "no alternative path found"
                        continue
                    old_path = path[:max_position]
                    print "change to:", old_path, "+", new_path
                    if (new_path[0],new_path[-1]) not in checking:
                        for sw in range(0, len(new_path)-1):
                            sw = new_path[sw]
                            datapath = self.dp_list[sw]
                            out_port = self.net[sw][new_path[new_path.index(sw)+1]]['port']
                            ofp_parser = datapath.ofproto_parser
                            actions = [ofp_parser.OFPActionOutput(out_port)]
                            src, dst = key
                            dst = "10.0.0.%s" % (str(dst))
                            match = ofp_parser.OFPMatch(eth_type=self.eth_type, ipv4_dst=dst)
                            self.add_flow(datapath, self.priority, match, actions, table=0)
                            self.priority = self.priority + 1
                        checking.append((new_path[0], new_path[-1]))

        print "Compute time:", (time.time() - start_time) * 1000, "ms"
        print "-----------------------------------\n"
        
    def WAR(self,index,path,net):
        new_path=None
        check_list=[]
        while (len(check_list)!=len(path)):
            max=-1
            max_position=-1
            for i in path:
                if self.weight[i]>max and i not in check_list:
                    max_position=path.index(i)
                    max=self.weight[i]
            if max_position<=index:
                try:
                    new_path=nx.shortest_path(net,path[max_position],path[-1])
                    for sw in path[:max_position]:
                        if sw in new_path:
                            continue
                    return new_path, max_position
                except:
                    log="do nothing"
            check_list.append(path[max_position])
        return None, -1
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst):
        ...
        match = parser.OFPMatch(eth_type=flow_info[0], ipv4_src=flow_info[1],
